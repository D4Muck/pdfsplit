...
def pdfcolorsplit(file, doublesided, merge, use_pdftoppm, verbose, lastpageinall):
    ...
    if merge:
        ...
        for (n, r) in enumerate(ranges):
            if lastpageinall and str(num_pages) not in r:
                r += ',' + str(num_pages)

            jobs.append((r, '%s_%03d%s' % (base_name, n + 1, suffixes[(n + iscolor[0]) % 2])))

    for (pages, name) in jobs:
        if verbose:
            print('qpdf "%s" --pages "%s" "%s" -- "%s"' % (file, file, pages, name))
        os.system('qpdf "%s" --pages "%s" "%s" -- "%s"' % (file, file, pages, name))
...
def usage():
    ...
    print('   -l Puts the last page of the original pdf as the last')
    print('      page in every produced pdf.')


def main():
    try:
        opt_pairs, filenames = getopt.gnu_getopt(sys.argv[1:], "hvpmsl", ["help"])
    ...
    lastpageinall = '-l' in opts
    for file in filenames:
        pdfcolorsplit(file, doublesided, merge, use_pdftoppm, verbose, lastpageinall)
